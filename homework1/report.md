...
呼叫 powerset 遞迴函數
## 申論及開發報告
本程式以遞迴方式生成集合的所有子集合（Powerset），透過系統性地探索每一種元素的取捨組合，完整呈現集合的所有可能子集。
在實作過程中，我們以陣列（array）作為暫存結構，用以保存當前的選擇狀態。遞迴函式在每一層呼叫中決定是否加入某個元素，並在抵達基底條件時輸出目前形成的子集合。此方法結構清晰、邏輯簡潔，能有效展示遞迴演算法的運作原理。
透過本次開發，我們不僅理解了子集合生成的邏輯流程，也加深了對遞迴函式呼叫堆疊與狀態回溯（backtracking）機制的掌握。
