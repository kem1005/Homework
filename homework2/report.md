...
- Mult 函式：這個函式的效能會較差，因為它使用一個迴圈嵌套另一個迴圈（$O(N_A N_B)$），並且在每次內部迴圈中都執行同類項的查找與合併操作，最後還使用慢速排序。
- Eval 函式：這個函式遍歷所有項次，並對每個項次使用 $\operatorname{pow}(f, e)$ 函式來計算冪次。效率主要取決於項數 $N$ 和最大指數 $E_{max}$。
