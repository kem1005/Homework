...
#### 測試輸出
```shell
加法結果: 6x^7 + 4x^5 + 2x^3 + 1x
乘法結果: 12x^10 + 14x^8 + 4x^6
```
## 效能量測
| 功能          | 時間複雜度           | 空間複雜度      | 說明                     |
| ----------- | --------------- | ---------- | ---------------------- |
...
void Polynomial::AddTerm(float c, int e) {
    ...
    for (int i = 0; i < terms; i++) {
        if (termArray[i].exp == e) {
            termArray[i].coef += c;
            if (termArray[i].coef == 0) { // 合併後為0，刪除此項
                for (int j = i; j < terms - 1; j++)
                    termArray[j] = termArray[j + 1];
                terms--;
            }
...
| 輸出資料 |  p(x)|p(4)| 
|----------|--------------|------|
| 所有資料統整  |        3x^4 + 2x^3 + 3x   |   438  |

### 編譯與執行指令
#### 測試輸入
```shell
輸入第一個多項式：
請輸入係數與指數（輸入 1010 結束）: 1 1 2 3
1010
輸入第二個多項式：
請輸入係數與指數（輸入 1010 結束）:
2 1 3 4
...
```shell
加法結果: 3x^4 + 2x^3 + 3x
乘法結果: 6x^7 + 3x^5 + 4x^4 + 2x^2
p1(6) = 438
```
## 效能量測
優化建議
- **改善排序**：將 `NewTerm` 中的泡沫排序替換為更高效的演算法，例如插入排序。由於每次只插入一個新項，插入排序比完整重新排序更高效。  
- **優化求值**：對於 `Evaluate`，可以使用 **霍納法則 (Horner's method)**，避免每次迴圈中重複計算 `pow()`，提高求值速度。
## 申論及開發報告
效能討論
- 目前程式的效能瓶頸主要在 `NewTerm()` 中的排序操作，當多項式項數增加時，時間複雜度可能達到 O(terms²)。  
- 對 `Evaluate()`，每次計算 `pow(x, exp)` 也會增加額外計算成本，對大指數或大量項次的多項式尤其明顯。  
- 加法與乘法操作中，每次都可能遍歷多項式並呼叫 `NewTerm()`，會累積排序與合併的開銷。
### 心得討論
- 本程式提供了一個基礎的多項式類別實作，展示了物件導向程式設計的核心概念。  
- 對於項數較多的多項式，其效能仍有待提升，特別是在 `NewTerm()` 的排序操作與 `Evaluate()` 的 pow 計算。  
- 透過替換低效排序演算法（如改用插入排序或在插入時保持排序）與使用霍納法則 (Horner's method) 來優化求值，可以明顯提升效率。  
- 本程式不僅展示功能，也說明其局限性，並提供明確的改進方向與效能優化建議。
