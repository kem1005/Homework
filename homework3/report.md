...
| 多項式加法 A + B      | O((n+m)^2)                  | 插入結果時可能遍歷已存在鏈表                     |
| 多項式減法 A - B      | O((n+m)^2)                  | 先取負再加法                                     |
| 多項式乘法 A * B      | O((n*m)^2)                  | 每項相乘並插入結果                               |
| Evaluate(x)           | O(n * max_exp) 可優化為 O(n * log(max_exp)) | 與幂運算和項數相關                               |
...
| 測試案例 | 輸入參數                | 預期輸出                               | 實際輸出                               | 
|----------|-------------------------|----------------------------------------|----------------------------------------|
| 測試一   | 3 3 3 2 2 0 | P1 = 3x^3 + 3x^2 + 2x^0               |P1 = 3x^3  + 3x^2  + 2x^0|
| 測試二   | 7 4 3 3 2 2 4 0      | P2 = 7x^4 + 3x^3 + 2x^2 + 4x^0       | P2 = 7x^4  + 3x^3  + 2x^2  + 4x^0 |
| P1 + P2  |                         | 7x^4 + 6x^3 + 5x^2 + 6x^0             | 7x^4 + 6x^3 + 5x^2 + 6x^0             |
| P1 - P2  |                         | -7x^4 + 1x^2 - 2x^0                   | -7x^4 + 1x^2 - 2x^0                   |
| P1 * P2  |                         | 21x^7 + 30x^6 + 15x^5 + 20x^4 + 18x^3 + 16x^2 + 8x^0 | 21x^7 + 30x^6 + 15x^5 + 20x^4 + 18x^3 + 16x^2 + 8x^0 |
| P1(2)    | x = 2                   | 38                                     | 38                                     |
| P2(2)    | x = 2                   | 148                                    | 148                                    |
### 測試輸入
```
...
```
## 申論及開發報告
本程式採用鏈結串列作為多項式的主要資料結構，每個節點表示多項式中的一項，包含係數與指數。由於多項式項數在執行期間可能動態變化且指數不連續，鏈結串列避免了陣列需預先配置大小或頻繁資料搬移的問題，提升結構彈性。 在記憶體管理方面，程式設計了 Available List（Free List）進行節點回收。整體而言，程式結合鏈結串列、Iterator 與 Available List，兼具結構彈性、運算效率與記憶體管理，實現功能完整且符資料結構原則的多項式運算系統。
### 結論
